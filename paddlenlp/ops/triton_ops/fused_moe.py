...
def get_default_config():
    #4090: default
    config = triton.Config({'BLOCK_SIZE_M':BLOCK_SIZE_M, 'BLOCK_SIZE_N':64, 'GROUP_SIZE_M':32,
                            },
...
def fused_moe_kernel_paddle(
    ...
    """
    # 实现方式
    # 按照MoE ID选需要的top token执行matmul
    # -----------------------------------------------------------
    # Map program ids `pid` to the block of C it should compute.
    # This is done in a grouped ordering to promote L2 data reuse.
    state_bit: tl.constexpr = 2
    pack_num: tl.constexpr = 3
    w_mask: tl.constexpr = 0xF
    bzp: tl.constexpr = 8
    real_k_size: tl.constexpr = (BLOCK_SIZE_K-1) // pack_num + 1
 
    pid = tl.program_id(axis=0)
    # even_Ks = tl.multiple(K, BLOCK_SIZE_K)
    # even_Ks: tl.constexpr = True
    # num_pid_m = tl.cdiv(EM, BLOCK_SIZE_M)
    # num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    # num_pid_m = tl.cdiv(EM, BLOCK_SIZE_M)
    # num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    ...
    offs_token_id = pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
    offs_token = tl.load(sorted_token_ids_ptr + offs_token_id)
    # tl.static_print('test')
    # tl.static_print(num_valid_tokens.dtype)
    token_mask = offs_token < num_valid_tokens
    ...
    # group_nums: tl.constexpr = (BLOCK_SIZE_K-1) // group_size + 1 # force to be 1 now
 
    offs_shift = BLOCK_SIZE_K + 1 - tl.arange(0, BLOCK_SIZE_K) # [65,2]
    # offs_bzk = tl.arange(0, group_nums)
 
 
 
    b_shift_bits = ((offs_shift[:,None] % pack_num) * state_bit).to(tl.int8)
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        b = (((b >> b_shift_bits) & w_mask) - bzp)* bs     # .to(a.dtype)
        ...
        a_ptrs += BLOCK_SIZE_K * stride_ak
        bs_ptrs += stride_bsk
    # if MUL_ROUTED_WEIGHT:size * stride_bk
    ...
    # -----------------------------------------------------------
    # Write back the block of the output
    # TODO continue here, how?
    offs_cn = pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
    c_ptrs = c_ptr + stride_cm * offs_token[:, None] + stride_cn * offs_cn[None, :]
    c_mask = token_mask[:, None] & (offs_cn[None, :] < N)
    # tl.static_print('test')
    # tl.static_print(c_mask.shape)
...
#         stride_bse,stride_bsk,stride_bsn = B_scale.shape[1]*B_scale.shape[2],B_scale.shape[2],1
#         num_valid_tokens = topk_ids.numel().tolist()

#         grid = lambda META: ((EM+META["BLOCK_SIZE_M"]-1)//META["BLOCK_SIZE_M"] * ((N+META["BLOCK_SIZE_N"]-1)//META["BLOCK_SIZE_N"]),)
...
def invoke_fused_moe_kernel(A , B , C ,
    ...
    K = A.shape[-1]
    N = B.shape[-1]
    EM = sorted_token_ids.shape[0] # num_tokens ？？？？
    stride_am,stride_ak = A.shape[1],1
    stride_be,stride_bk,stride_bn = B.shape[1]*B.shape[2], B.shape[2], 1
    stride_cm,stride_cn = B.shape[2],1
    stride_bse,stride_bsk,stride_bsn = B_scale.shape[1]*B_scale.shape[2],B_scale.shape[2],1
    num_valid_tokens = topk_ids.numel().tolist()
...
#         fused_moe_kernel_paddle[(grid, configs)](
#             A,
#             B,
#             C,
#             A_scale,
...
def fused_experts_impl(
    ...
    # 顺序记录属于每个专家，的token所在的行数
    # 每个block对应的专家id
    # 按照block填充后的总的token数量
    # breakpoint()
...
def fused_moe(
    ...
    topk_weights, topk_ids = paddle.topk(scores, k=topk, axis=-1, sorted=False) #前八个专家 每个专家的分数，和token ID
    # breakpoint()
